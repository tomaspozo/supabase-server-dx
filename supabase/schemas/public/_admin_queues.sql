...
-- @signature _admin_queue_read(qty integer DEFAULT 5, vt integer DEFAULT 30)
...
-- @example SELECT * FROM _admin_queue_read(10, 60);
-- @related agentlink_tasks, _admin_queue_delete, _admin_queue_archive, queue-worker
CREATE OR REPLACE FUNCTION public._admin_queue_read(
...
REVOKE ALL ON FUNCTION public._admin_queue_read(integer, integer) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._admin_queue_read(integer, integer) TO service_role;
...
-- @signature _admin_queue_delete(id bigint)
-- @returns boolean
-- @security SECURITY DEFINER — only service_role can execute
-- @example SELECT _admin_queue_delete(42);
-- @related agentlink_tasks, _admin_queue_read, _admin_queue_archive
CREATE OR REPLACE FUNCTION public._admin_queue_delete(id bigint)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = ''
...
REVOKE ALL ON FUNCTION public._admin_queue_delete(bigint) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._admin_queue_delete(bigint) TO service_role;
...
-- @signature _admin_queue_archive(id bigint)
-- @returns boolean
-- @security SECURITY DEFINER — only service_role can execute
-- @example SELECT _admin_queue_archive(42);
-- @related agentlink_tasks, _admin_queue_read, _admin_queue_delete, queue-worker
CREATE OR REPLACE FUNCTION public._admin_queue_archive(id bigint)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = ''
...
REVOKE ALL ON FUNCTION public._admin_queue_archive(bigint) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._admin_queue_archive(bigint) TO service_role;
