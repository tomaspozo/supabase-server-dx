...
--   api._admin_enqueue_task and processed by the queue-worker edge function.
...
-- @description Fires the queue-worker edge function every minute via pg_cron.
...
--   retried within 60 seconds.
-- @related _admin_enqueue_task, queue-worker, agentlink_tasks
SELECT cron.schedule(
  'process-stale-tasks',
  '* * * * *',
  $$SELECT public._internal_admin_call_edge_function('queue-worker')$$
);
